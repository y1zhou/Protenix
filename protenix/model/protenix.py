...
class Protenix(nn.Module):
    ...
    def main_inference_loop(
        ...
        Args:
            ...
            score_only (bool): If True, skip diffusion sampling and score confidence using provided coordinates.
            x_pred_coords (Optional[torch.Tensor]): Coordinates to score when score_only=True. Shape:
                [N_atom,3] or [N_sample,N_atom,3].
    ...
    def _main_inference_loop(
        ...
        if score_only:
            ...
            pred_dict["coordinate"] = coords.to(device=s_inputs.device, dtype=s_inputs.dtype)
